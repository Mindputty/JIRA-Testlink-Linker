...
require_once('../api/xmlrpc/v1/sample_clients/php/jira_test.php');
...
function write_execution(&$db,&$exec_signature,&$exec_data,&$issueTracker)
{
  ...
  foreach ( $item2loop as $tcversion_id => $val)
  {
    ...
    { 
      ...
      // Update the execution link on JIRA to reflect the status update
      $jira_args['tcversion_id'] = $tcversion_id;
      $jira_args['build_id'] = $exec_signature->build_id;
      $jira_args['status'] = $current_status;
      $jira_args['tproject_id'] = $exec_signature->tproject_id;
      update_status_on_jira($jira_args);
